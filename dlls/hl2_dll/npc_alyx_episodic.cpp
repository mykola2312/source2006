...
BEGIN_DATADESC( CNPC_Alyx )
	...
	DEFINE_FIELD( m_bHasShotgun, FIELD_BOOLEAN ),
...
{
	CBaseEntity *pEnt = gEntList.FindEntityByClassname( NULL, "npc_alyx" );
	CNPC_Alyx *pAlyx = static_cast<CNPC_Alyx *>( pEnt );
	return pAlyx;
}
...
void CNPC_Alyx::ReadinessLevelChanged( int iPriorLevel )
{
	...
	if ( npc_alyx_readiness_transitions.GetBool() )
	{
		...
		for ( int i = 0; i < ARRAYSIZE( readinessTransitions ); i++ )
		{
			...
			{
				RestartGesture( translatedTransitionActivity );
				break;
			}
		}
	}
		...
		if( (pSound->SoundTypeNoContext() & SOUND_COMBAT) && (pSound->SoundContext() & SOUND_CONTEXT_EXPLOSION) )
		{
			if ( m_bHasShotgun )
			{
				if ( !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_SHOTGUN) && !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN) )
				{
					...
					GetShotRegulator()->FireNoEarlierThan( gpGlobals->curtime + SequenceDuration( ACT_GESTURE_FLINCH_BLAST_SHOTGUN ) );
				}
			}
			else
			{
				if ( !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) && !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED) )
				{
					...
					GetShotRegulator()->FireNoEarlierThan( gpGlobals->curtime + SequenceDuration( ACT_GESTURE_FLINCH_BLAST ) );
				}
			}
		}
	}
...
void CNPC_Alyx::Event_KilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
	...
	if( !m_bHasShotgun )
	{
		CAI_BaseNPC *pTarget = CreateCustomTarget( pVictim->GetAbsOrigin(), 0.5f );
...
//-----------------------------------------------------------------------------
// Purpose: Called by player code when player kills another entity
// Input  : pVictim - entity killed by player
//-----------------------------------------------------------------------------
void CNPC_Alyx::PlayerKilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_Alyx::IsNavigationUrgent( void )
{
	bool bBase = BaseClass::IsNavigationUrgent();

	// Alyx considers follow & assault behaviour urgent
	if ( !bBase && (m_FollowBehavior.IsActive() || ( m_AssaultBehavior.IsRunning() && m_AssaultBehavior.IsUrgent() )) ) 
	{
		...
		// Avoid teleporting right in front of the player
		if ( HasCondition(COND_SEE_PLAYER) )
		{
...
void CNPC_Alyx::DoCustomCombatAI( void )
{
	...
	if( pEnemy && GetAbsOrigin().DistToSqr( pEnemy->GetAbsOrigin() ) < Square( 60.0f ) )
	{
		// Don't reload if an enemy is right in my face, either.
		ClearCondition( COND_LOW_PRIMARY_AMMO );
	}
...
Activity CNPC_Alyx::NPC_TranslateActivity( Activity activity )
{
	...
	if ( activity == ACT_PICKUP_RACK )
		return (Activity)ACT_ALYX_PICKUP_RACK;

	if ( activity == ACT_DROP_WEAPON && m_bHasShotgun )
		return (Activity)ACT_DROP_WEAPON_SHOTGUN;
	...
	case TASK_ALYX_HOLSTER_AND_DESTROY_PISTOL:
		{
			...
			if ( GetActiveWeapon() && FClassnameIs(GetActiveWeapon(), "weapon_alyxgun") )
			{
	...
	if( taken && (info.GetDamageType() & DMG_BLAST) )
	{
		if ( m_bHasShotgun )
		{
			if ( !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) && !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN) )
			{
				...
				GetShotRegulator()->FireNoEarlierThan( gpGlobals->curtime + SequenceDuration( ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN ) );
			}
		}
		else
		{
			if ( !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) && !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED) )
			{
				...
				GetShotRegulator()->FireNoEarlierThan( gpGlobals->curtime + SequenceDuration( ACT_GESTURE_FLINCH_BLAST_DAMAGED ) );
			}
		}
	}
...
void CNPC_Alyx::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget, const Vector *pVelocity )
{
	...
	if( pWeapon && pWeapon->ClassMatches("weapon_alyxgun") )
	{
		pWeapon->SUB_Remove();
	}
}
...
bool CNPC_Alyx::BlindedByFlare( void )
{
	...
	while( pFlare != NULL )
	{
		...
		{
			// Check facing to ensure we're in front of her
			los = ( pFlare->GetAbsOrigin() - EyePosition() );
			los.z = 0;
			VectorNormalize( los );
...
//-----------------------------------------------------------------------------
// Purpose: Translate damage to heavy damage under certain circumstances
// Output : Returns true if the damage should be considered... considerable
//-----------------------------------------------------------------------------
bool CNPC_Alyx::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
{
	if( m_bHasShotgun && GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE && random->RandomInt( 0, 1 ) == 1 )
	{
...
{
	bool bOutsideVehicle = ( m_PassengerBehavior.GetPassengerState() == PASSENGER_STATE_OUTSIDE );
	
	// Only do readiness if we're outside the vehicle
	if ( m_PassengerBehavior.CanSelectSchedule() && bOutsideVehicle == false )
		return false;
...
void CNPC_Alyx::OnGivenWeapon( CBaseCombatWeapon *pNewWeapon )
{
	...
	if ( FClassnameIs( pNewWeapon, "weapon_alyxgun" ) )
	{
