...
void CHudHDRDemo::Paint()
{
	...
	// Left Title
 	wchar_t *tempString = vgui::localize()->Find("#Valve_HDRDEMO_LeftTitle");
	if (tempString)
	{
		int iLength = 0;
	...
	// Right Title
	tempString = vgui::localize()->Find("#Valve_HDRDEMO_RightTitle");
	if (tempString)
	{
		int iLength = 0;
		for ( wchar_t *wch = tempString; *wch != 0; wch++ )
...
{ 
	...
	{
		ConVar *pHideHud = (ConVar *)cvar->FindVar( "hidehud" );
...
void mat_show_ab_hdr_hudelement_changed( ConVar *var, const char *pOldString )
{
